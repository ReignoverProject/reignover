...
contract Research is Editor {
    ...
    uint baseForgeMod = 2500; // based on the max level of forges at max cities (lvl 100 * 25 cities)
    ...
    uint researchMaxLevel = 100; // default max research level - same for all research
    ...
    event NewResearchMaxLevel(uint maxLevel);
    ...
    */ 
    function addResearch(string memory _name, uint[] memory _levelRequirements, uint[] memory _resourceRequirements) external onlyOwner {
    ...
    */
    ...
    function setResearchMaxLevel(uint _maxLevel) external onlyOwner {
        researchMaxLevel = _maxLevel;
        emit NewResearchMaxLevel(_maxLevel);
    }
    ...
    */
    ...
    function getNextLevelTimeRequirement(uint[] memory _playerResearchLevels, uint _researchId) public view returns(uint) {
        ...
        uint forgeMod = Kingdoms.getTotalForges(msg.sender);  
        uint timeCost;
        for (uint i = 0; i < resources.length; i++) {
            timeCost += resourceCost[i] * i;
        }
        return (timeCost * baseForgeMod - (forgeMod * timeCost / 2)) / baseForgeMod;
    }
    ...
    /** @notice adjusts the research time modifier if max buliding level changes  */
    function updateBaseForgeMod(uint _maxBuildingLevel) external onlyOwner {
        baseForgeMod = _maxBuildingLevel * researchMaxLevel;
    }
}